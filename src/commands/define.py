...
class DefineCommand(Command):
    ...
    def __init__(self, ffmpeg_path: pathlib.Path):
        super().__init__('define', aliases=['d'], description='Gets the definition of a word.', usage='<word>')
        self._definition_response_manager = DefinitionResponseManager(ffmpeg_path)
...
class DefinitionResponseManager:
    ...
    def add(self, word, client, message: discord.Message, reverse=False, text_to_speech=False):
        """
        Add a request
        :param word:
        :param message:
        ...
        if text_channel not in self._request_queues:
            self._request_queues[text_channel] = MessageQueue(client, self._ffmpeg_path)
