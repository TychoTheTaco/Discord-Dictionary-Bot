...
class DictionaryBotClient(discord.Client):
    ...
    def __init__(self, *, loop=None, **options):
        ...
        self._commands = [
            Command('define', aliases=['d', 'b', 'dv', 'bv'], description='Prints the definition of the word in chat and if you are in a voice channel, reads it out.', arg_format='<word>'),
        ...
        for command in self._commands:
            if command.matches(command_input[0]):
                if command._name == 'help':
                    ...
                    print(command_input[0], text_to_speech, len(command_input[0]), command_input[0][1])
